 """사용자 도메인 엔드포인트 테스트 (#9-17, #43, #50)."""
  ...
  class TestUpdateBody:
      ...
      @pytest.mark.asyncio
      async def test_update_height_no_profile_returns_400(self, client):
  ...
  class TestUpdateGoal:
      ...
      @pytest.mark.asyncio
      async def test_profile_not_found_returns_400(self, client):
          db = _make_db(_exec_scalar(None))
          app.dependency_overrides[get_db] = _db_override(db)
  ...
  class TestAddPrimaryGym:
      ...
      async def test_gym_not_found(self, client):
          ...
          resp = await client.post("/api/v1/users/me/gym", json={"gym_id": str(uuid.uuid4())})
  ...
  class TestList1RM:
      ...
      @pytest.mark.asyncio
      async def test_success_with_records(self, client):
          orm = MagicMock(spec=UserExercise1RM)
          orm.id = uuid.uuid4()
          orm.exercise_id = _EXERCISE_ID
          orm.weight_kg = 100.0
          orm.source = MagicMock()
          orm.source.value = "manual"
          ...
          assert data["items"][0]["weight_kg"] == 100.0
          assert data["items"][0]["exercise_name"] == "벤치 프레스"
  ...
  class TestAdd1RM:
      ...
      async def test_success(self, client):
          ...
          resp = await client.post(
              "/api/v1/users/me/1rm",
              json={"exercise_id": str(_EXERCISE_ID), "weight_kg": 80.0},
          )

          assert resp.status_code == 201
          assert resp.json()["data"]["weight_kg"] == 80.0
      ...
      async def test_exercise_not_found(self, client):
          ...
          resp = await client.post(
              "/api/v1/users/me/1rm",
              json={"exercise_id": str(uuid.uuid4()), "weight_kg": 80.0},
          )
  ...
  class TestUpdate1RM:
      ...
      async def test_success(self, client):
          ...
          resp = await client.patch(
              "/api/v1/users/me/1rm",
              json={"exercise_id": str(_EXERCISE_ID), "weight_kg": 110.0},
          )

          assert resp.status_code == 200
          assert resp.json()["data"]["weight_kg"] == 110.0
