...
def iter_records(path: Path) -> Iterator[tuple[Chunk, list[float]]]:
...
def load_local(input_path: Path, batch_size: int) -> int:
    """로컬 ChromaDB(PersistentClient)에 직접 적재."""
    logger.info("로컬 ChromaDB 적재 시작: %s", input_path)
...
def load_api(input_path: Path, batch_size: int) -> int:
    ...
    logger.info("서버 API 적재 시작: POST %s", url)
...
def main(input_path: Path, mode: str, batch_size: int) -> None:
    if not input_path.exists():
        logger.error("입력 파일 없음: %s", input_path)
        sys.exit(1)

    if mode == "local":
        total = load_local(input_path, batch_size)
    elif mode == "api":
        total = load_api(input_path, batch_size)
    else:
...
if __name__ == "__main__":
    ...
    main(args.input, args.mode, args.batch_size)
