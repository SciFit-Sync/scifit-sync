...
| transient NCBI 에러 재시도 (ChunkedEncoding/5xx/429) | `pytest .../TestRequestWithRateLimit` 7건, 실측 PMC 성공률 0/3→2/3 |
...
# 기대: 24 passed (parse 4 + search 2 + get_text 3 + round-robin 8 + retry 7)
```
...
### NCBI 응답 transient 에러

`commit a14d0ab`에서 ChunkedEncodingError/Timeout/5xx에 대한 3회 재시도가 자동 적용된다. 그래도 실패하는 PMID는 `crawl_papers`가 abstract fallback으로 처리 → 최종 결과에서 청크 손실 없음.
