...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `MAX_PAPERS_PER_RUN` | `300` | 1회 실행에서 처리할 PMID 최대 수. entry script의 `--max-papers` 미지정 시 fallback |
| `OPENALEX_MAX_PER_CATEGORY` | `500` | 카테고리당 OpenAlex 후보 풀 cap |
| `PUBMED_MAX_PER_CATEGORY` | `50` | 카테고리당 PubMed 후보 풀 cap |
| CLI `--max-per-category` | 없음 | 명시 시 위 두 값을 양쪽 동일하게 override. 후보 풀이 클수록 round-robin이 부족 카테고리를 다른 카테고리에서 흡수할 여유가 커진다 |
...
# 카테고리당 후보 풀 cap은 CLI `--max-per-category`로 제어 (env 변수 아님)
# 생략 시 소스별 기본값 사용: OPENALEX_MAX_PER_CATEGORY=500 / PUBMED_MAX_PER_CATEGORY=50
```
...
```bash
# CLI로 직접 지정 (양쪽 소스에 동일 cap 적용 — 권장)
...
# 또는 소스별로 다르게 두려면 환경변수로 분리 (CLI 생략 시 fallback)
export OPENALEX_MAX_PER_CATEGORY=500
export PUBMED_MAX_PER_CATEGORY=50
```
...
# 카테고리당 cap은 run script 안에서 CLI `--max-per-category`로 지정 (env 미사용)
```
