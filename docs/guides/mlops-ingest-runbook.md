# MLOps 클라우드 Ingest 운영 Runbook (50 카테고리 기준)
...
```bash
# === NCBI ===
# 요청 간격 1.5s → 1.0s, PMC 전문 회수율 +20%p
...
# === ingest 파라미터 (환경변수는 entry script의 fallback일 뿐 — CLI 옵션이 우선) ===
export MAX_PAPERS_PER_RUN=2000          # 한 번에 신규 PMID 상한
# 카테고리당 후보 풀 cap은 CLI `--max-per-category` 로 지정 (예: `--max-per-category 50`).
# 생략 시 소스별 기본값(OPENALEX_MAX_PER_CATEGORY=500 / PUBMED_MAX_PER_CATEGORY=50) 사용.

# === ChromaDB / 임베딩 (RAG 서비스도 같은 값을 봐야 함) ===
# 운영 컬렉션의 SOT는 EFS /chroma-data/current_alias.json (alias-swap) —
# 운영 적재 전 phase2-execution-runbook.md §0.4.1 절차로 현재 alias 확인 후 일치시킬 것
...
운영 ChromaDB에 옛 (29 / 100 / 65 카테고리 시절) 메타가 남아있다면 현행 50 기준으로 재매핑:
...
> 한 번만 돌리면 끝. 다음 monthly ingest는 자동으로 새 메타로 적재됨.
...
curl -X POST "$API_BASE_URL/api/v1/chat/messages" \
    ...
    -d '{"message": "벤치프레스 근비대를 위한 적정 세트와 반복은?"}'
```

응답 SSE 스트림에 논문 출처 카드가 포함되면 정상.

> `session_id`는 생략하면 새 세션이 생성된다. 지정하려면 본인 소유 세션의 **UUID**여야 함 — 임의 문자열(`"test-runbook"` 등)은 검증 에러(400).
...
| 카테고리당 후보 풀이 너무 작음 | `--max-per-category` 미지정 시 PubMed cap이 50으로 묶임 | `--max-per-category 100` 같은 큰 값으로 CLI 명시 |
...
| **매월 자동** | GitHub Actions `mlops.yml` (`monthly_ingest.py`로 신규 35일치 PMID 추가) |
