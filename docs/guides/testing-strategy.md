...
> 핵심 계산 로직(`load_calc`·1RM·Progressive Overload)은 커버리지 100% 달성·유지를 원칙으로 한다. CI는 커버리지를 측정·보고만 하며(fail-under 없음) 커버리지 수치로 머지를 차단하지 않는다.
...
> **현재 작성된 테스트 0건** — 아래는 테스트 작성 시 적용할 기준.
