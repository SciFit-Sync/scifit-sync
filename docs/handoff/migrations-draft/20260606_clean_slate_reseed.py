...
⚠️⚠️ DRAFT — 자동 실행 금지. server/alembic/versions/ 로 옮기기 전 아래 선결 필수 ⚠️⚠️
  ...
  2) 모델/코드 동반 변경 필수: 이 마이그가 DROP하는 컬럼/테이블을 ORM이 아직 참조한다
     (default_equipment_id, equipment_muscles/EquipmentMuscle, movement_label_*, is_freeweight,
      exercise_equipment_map/ExerciseEquipmentMap). 본 마이그만 적용하고 앱을 띄우면 import/쿼리
      단계에서 크래시. 정합성 분리지점 142건 = docs/handoff/2026-06-06-code-coupling-report.md.
  3) down_revision 위치: 재시드 시드 마이그레이션보다 **반드시 선행**(PHASE3가 재시드 데이터를
     재삭제하는 1회성 제약). 재시드 마이그는 이 revision 뒤에 체인.
