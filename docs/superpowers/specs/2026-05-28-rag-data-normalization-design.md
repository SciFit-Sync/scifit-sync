...
| ID | 조건 | 측정 |
|---|---|---|
| C1 | prod ChromaDB의 모든 청크에 `paper_doi` 100% 존재, PMID fallback 청크 0개 | ChromaDB metadata sampling |
...
| C5 | 골드셋 recall@10이 현재 baseline 대비 회귀 없음 (가능하면 향상) | A/B 평가 리포트 |
| C6 | 파이프라인이 resumable — 중간 중단 후 재실행 시 멱등 동작 | 단위 테스트 + 운영 검증 |
| C7 | prod 적용 시 다운타임 ≤ 5분 (alias swap 사용 시) | 운영 절차로 보장 |
| C8 | pre-upsert validation 게이트 합격 — §3.3의 9개 항목 모두 임계 충족 | `validate_embeddings.py` |
...
└──┬───────────────────────────────────────────────────────────────┘
   ...
   │
   ├─► [Stage 1.5: Manifest sanity 검증]
   │     publication_types fill rate ≥ 90%, paper_doi 100%
   ...
   │     validate_embeddings.py — jsonl 9개 항목 임계 검증
...
| `ncbi.py` efetch publication_types | `mlops/pipeline/ncbi.py` (추정) | PubMed XML에서 `PublicationType` 추출 | **수정/디버그** (S5) |
...
- key: paper DOI (primary). value: `{pmid, pmcid, openalex_id, fulltext_source, publication_types, evidence_weight, tried_sources, chunked_at, embedded_at, upserted_at, batch_tag}`
...
- 전체 적재·검증 완료 후 admin endpoint(`/admin/rag/swap?to=papers_v2`)로 ChromaDB 메타 alias 변경
- rag.py가 ChromaDB collection 핸들을 매 요청마다 lookup하도록 (현재 모듈 글로벌 캐시 → 수정 필요)
- 롤백: alias 되돌리기로 즉시 복구
...
| P1-2 | prod ChromaDB 와이프 (EFS `/chroma-data` + ECS force-new-deployment) | 기존 38,800 청크 폐기. 메모리 `project_prod_rag_full_reload_20260528` 절차 |
...
| **PR-ε** (S4-2, B2) | `/admin/rag/pmids` 페이지네이션 + graceful shutdown hook | `fix/jingyu/admin-pmids-pagination` | 독립 |
...
| **E2-3.5** | **Manifest sanity 검증 (paper-level)** — crawl 직후 manifest에서 publication_types 비어있지 않은 paper 비율 ≥ 90%, paper_doi 100%. 미달 시 abort → S5 디버그 재방문 (전체 fetch 재실행보다 먼저 차단). 임베딩 비용 절감용 early gate. | 1분 |
| E2-4 | Stage 2 chunk + Stage 3 embed (shard 단위 streaming) | 4~8시간 (~2.1M 청크 → 새 chunker로 ~400k 청크 감소 예상) |
| **E2-4.5** | **Pre-Upsert Validation 게이트 (chunk-level)** — `validate_embeddings.py`로 임베딩 jsonl 9개 항목 임계 검증. paper→chunk 전파 + 임베딩 차원 검증까지 포함. E2-3.5는 paper 메타 기준, E2-4.5는 청크/임베딩 산출물 기준. 미달 시 abort, ChromaDB 진입 금지. | 5분 |
| E2-5 | Stage 4 upsert to `papers_v2` collection | 6~12시간 |
| E2-6 | Stage 5 평가 게이트 — `run_eval`로 recall@10 A/B (`papers` vs `papers_v2`) | 30분 |
| E3 | 합격 시 `gh api /swap?to=papers_v2` 또는 운영자 명령 | 1분 (다운타임 ≤ 5분) |
| E4 | 1주 후 옛 `papers` 컬렉션 정리 | 별도 |
...
| 검증 항목 | 임계 |
|---|---|
| **스키마**: 키 11개 모두 존재 (`chunk_index`, `paper_pmid`, `paper_title`, `section_name`, `token_count`, `search_categories`, `paper_doi`, `publication_types`, `evidence_weight`, `fulltext_source`, `published_year`, `embedding`) | 100% |
| **paper_doi fill rate** | ≥ 99% (local_pdf 일부 예외 허용) |
...
schema:                   ✅ 11/11 keys present, 0 missing
paper_doi fill rate:      ✅ 99.8% (5/2,341 missing — local_pdf만)
...
| 시나리오 | 조치 | 손실 |
|---|---|---|
| Phase 1 적재 후 chat 응답 회귀 | 와이프 + 빈 상태 유지 (또는 옛 jsonl 재적재) | 데모 임시 중단 |
...
| Phase 2 alias swap 후 회귀 발견 | `/admin/rag/swap?to=papers` 즉시 원복 | swap 시간만 |
...
# 2. prod ChromaDB 와이프 (AWS 콘솔 또는 ECS exec — 사용자 확정 후 진행)
...
| 평가 게이트 회귀 (recall@10 하락) | C2 임계(publication_types 90%) 달성 못한 경우 PR-α 디버그 재방문. chunk size만 정상화돼도 baseline 회복 가능성 있음 |
...
| efetch publication_types 픽스(S5)가 어려운 경우 | **Pre-Upsert 게이트 임계(§3.3.1)는 운영 fallback으로 70%까지 일시 완화 가능** (incident response 차원). 단, **§1 C2 90% / alias swap 합격 임계는 변경 금지** — fallback으로 임시 적재 후 S5 픽스 PR로 재실행 |
...
- PR #174 (chunker packing) — ✅ 2026-05-28 머지 완료
