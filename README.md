...
| 역할 | 담당 |
|------|------|
| **Team Lead** — MLOps Lead & System Architect | 전체 시스템 아키텍처 및 백엔드 초기 프레임워크 설계, MLOps 파이프라인(크롤링/임베딩/적재) 구축, AI/Data 파트 교차 지원 및 최적화 |
| **AI/Data Engineer** | RAG 파이프라인 구축 및 최적화, LLM(Gemini) 프롬프트 엔지니어링 및 응답 품질 개선, 스포츠 과학 논문 데이터 분석 |
| **AI/Data Engineer** (Cross-functional) | AI/Data 파트 데이터 전처리 및 모델 고도화 보조, MLOps 데이터 파이프라인 로직 지원 등 AI 인프라 전반의 교차 지원 |
| **Backend Engineer** | FastAPI 기반 핵심 비즈니스 로직 및 API 개발, 운동 기록 및 루틴 도메인 구현, SSE 스트리밍 엔드포인트 구축 |
| **Backend Engineer** | 사용자 인증(JWT/OAuth) 및 권한 관리 로직 구현, 헬스장/기구 도메인 외부 API 연동, 데이터베이스 모델링 및 에러 핸들링 고도화 |
| **Frontend Engineer** | React Native(Expo) 기반 모바일 앱 UI/UX 개발, 전역 상태 관리(Zustand) 및 서버 데이터 동기화(TanStack Query), 클라이언트 사이드 비즈니스 로직 구현 |
